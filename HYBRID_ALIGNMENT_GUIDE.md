...
#### **Step 1: Scan and Group Chunks by Folder Structure**

**IMPORTANT:** The folder structure defines which chunks belong to which camera - don't rely solely on filename parsing!
...
Input: Processed_Videos/P01_20250709/
  ...
  │   ├── P01_Cam1_chunk008.mp4
  │   ├── P01_Cam1_chunk009.mp4
  │   └── P01_Cam1_chunk010.mp4
  ├── Cam2_X5/
  │   ├── P01_Cam2_chunk004.mp4
  │   ├── P01_Cam2_chunk005.mp4
  │   └── P01_Cam2_chunk006.mp4
  └── Cam3_X5/
      ├── P01_Cam3_chunk004.mp4
      ├── P01_Cam3_chunk005.mp4
      └── P01_Cam3_chunk006.mp4

Grouping logic:
  1. Scan each camera folder (Cam1_X4/, Cam2_X5/, Cam3_X5/)
  2. Extract camera ID from FOLDER NAME (not filename!)
  3. Extract chunk number from filename
  4. Sort chunks by chunk number within each camera

Result:
  Cam1_X4: 4 chunks (007, 008, 009, 010) - All in Cam1_X4/ folder
  Cam2_X5: 3 chunks (004, 005, 006) - All in Cam2_X5/ folder
  Cam3_X5: 3 chunks (004, 005, 006) - All in Cam3_X5/ folder

✅ Folder structure = ground truth for camera grouping!
```

**Benefits of folder-based grouping:**
- ✅ **Unambiguous** - Folder name clearly identifies camera
- ✅ **Handles any filename** - Don't need perfect filename parsing
- ✅ **User-controlled** - Researcher organizes chunks by camera
- ✅ **Robust** - Works with different naming conventions

#### **Step 2: Audio Alignment of ALL Chunks**

**IMPORTANT:** The system audio-aligns **EVERY chunk** (not just first chunks) for maximum accuracy.

```
Extracts audio from ALL chunks across ALL cameras:
  Cam1_X4/chunk007.mp4 → Audio pattern analysis
  Cam1_X4/chunk008.mp4 → Audio pattern analysis
  Cam1_X4/chunk009.mp4 → Audio pattern analysis
  Cam1_X4/chunk010.mp4 → Audio pattern analysis
  Cam2_X5/chunk004.mp4 → Audio pattern analysis
  Cam2_X5/chunk005.mp4 → Audio pattern analysis
  Cam2_X5/chunk006.mp4 → Audio pattern analysis
  Cam3_X5/chunk004.mp4 → Audio pattern analysis
  ...

Cross-correlates ALL chunks against reference to find exact positions:
  Cam1_X4_chunk007: 0.0s (earliest start) ← REFERENCE
  Cam2_X5_chunk004: 75.2s (audio correlation)
  Cam3_X5_chunk004: 92.5s (audio correlation)
  Cam1_X4_chunk008: 1800.1s (audio correlation) ✅ Actual audio position
  Cam1_X4_chunk009: 3600.2s (audio correlation)
  Cam1_X4_chunk010: 7137.0s (audio correlation) [GAP detected!]
  ...
```

**Why audio-align everything?**
- ✅ **No duration assumptions** - File metadata can be inaccurate
- ✅ **Detects overlaps** - Catches recording errors automatically
- ✅ **Finds true gaps** - Audio reveals actual timeline
- ✅ **Self-correcting** - Errors don't compound across chunks
- ✅ **More robust** - Works even with truncated/corrupted files

#### **Step 3: Filename-Based Gap Verification**

After audio alignment, use **filenames to verify gaps** (not to determine position):

##### **Tier 1: Audio Alignment (Primary Method)**

```python
# Audio-align ALL chunks to reference
for chunk in all_chunks:
    chunk.audio_offset = cross_correlate_with_reference(chunk.audio)

# Sort chunks by camera and chunk number
camera_chunks = group_by_camera(all_chunks)

# Example results:
Cam1_chunk007: 0.0s (audio)
Cam1_chunk008: 1800.1s (audio)
Cam1_chunk009: 3600.2s (audio)
Cam1_chunk010: 7137.0s (audio)  ← Big jump!
```

##### **Tier 2: Filename Timestamp Verification**

```python
# Extract timestamps from filenames to verify gaps
for i, chunk in enumerate(chunks[1:], 1):
    prev_chunk = chunks[i-1]

    # Calculate expected position (if continuous)
    expected_offset = prev_chunk.audio_offset + prev_chunk.duration

    # Compare with actual audio position
    actual_offset = chunk.audio_offset
    gap = actual_offset - expected_offset

    # Check filename timestamp (if available)
    if has_timestamp(chunk.filename):
        chunk_time = extract_timestamp(chunk.filename)
        prev_chunk_time = extract_timestamp(prev_chunk.filename)
        timestamp_delta = (chunk_time - prev_chunk_time).total_seconds()

        # Cross-verify audio gap with timestamp gap
        if abs(gap) < 2.0:
            print(f"✅ Continuous (gap < 2s)")
            chunk.gap_detection_method = "audio_continuous"

        elif abs(gap - timestamp_delta) < 5.0:
            # Audio gap matches timestamp gap → real gap
            print(f"⚠️ Gap detected: {gap:.1f}s (verified by timestamp)")
            chunk.has_gap_before = True
            chunk.gap_duration = gap
            chunk.gap_detection_method = "timestamp_verified_gap"

        elif timestamp_delta < 5 and abs(gap) > 5:
            # Timestamp says continuous, but audio says gap
            print(f"🔍 Suspicious: timestamp={timestamp_delta:.1f}s, audio_gap={gap:.1f}s")
            chunk.gap_detection_method = "audio_gap_unexpected"

        else:
            # Large gap confirmed by both
            print(f"🚨 Large gap: {gap:.1f}s ({gap/60:.1f} min)")
            chunk.has_gap_before = True
            chunk.gap_duration = gap
            chunk.gap_detection_method = "audio_and_timestamp_gap"
```

##### **Tier 3: Fallback (No Timestamp)**

If filename doesn't contain timestamp:
```python
# Use audio-derived gap
if abs(gap) > 2.0:
    print(f"⚠️ Gap detected: {gap:.1f}s (audio-only, no timestamp)")
    chunk.has_gap_before = True
    chunk.gap_duration = gap
    chunk.gap_detection_method = "audio_gap_no_timestamp"
```

**Key Principle:**
- **Audio alignment determines POSITION** (most accurate)
- **Filename timestamps verify GAPS** (cross-check for errors)
