#!/usr/bin/env python3
"""
Test 2D Bad Gesture Detection with Webcam
Simple script to verify the 2D detection system works correctly
"""
...
def print_detailed_analysis(detector):
    ...
    print("\n" + "="*80)
    print("DETAILED POSTURE ANALYSIS (2D Detection)")
    print("="*80)
...
def test_2d_detection_webcam():
    """Main test function for webcam testing"""

    print("="*80)
    print("2D BAD GESTURE DETECTION - WEBCAM TEST")
    print("="*80)
    ...
    try:
        ...
        # Initialize 2D bad gesture detector
        bad_gesture_detector = BadGestureDetector()
    ...
    try:
        while True:
            ...
            # Detect pose (use 2D landmarks)
            pose_results = pose_detector.detect(frame)
            pose_landmarks_2d = pose_results  # MediaPipe pose results object (contains pose_landmarks)

            # Detect hands (use 2D landmarks)
            hand_results = hand_detector.detect(frame)
            hand_landmarks_2d = hand_results.multi_hand_landmarks if hand_results else None

            # Detect bad gestures using 2D screen coordinates
            ...
            cv2.putText(frame, "Mode: 2D (Screen Coords)", (10, frame.shape[0] - 40),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.5, (100, 255, 255), 1)

            # Show frame
            cv2.imshow('2D Bad Gesture Detection - Webcam Test', frame)
        ...
        print("\n" + "="*80)
        print("SESSION SUMMARY (2D Detection)")
        print("="*80)
