...
Note: Choose the video with the earliest matching pattern (most concise content) 
as the reference to ensure only extra intro content is trimmed from longer videos.
"""
...
try:
    ...
    print("Warning: dtaidistance not available. DTW functionality will be limited.")
    dtw = None
import subprocess
import os
import sys
...
def find_dtw_offset(energy1, energy2, time_axis1, time_axis2, search_range=(8, 15)):
    ...
    for offset_samples in range(min_offset_samples, min(max_offset_samples, len(energy2_norm) - comparison_length)):
        ...
        if len(segment) == len(energy1_segment):
            ...
            if dtw is not None:
                ...
                # Fallback to simple correlation distance if DTW not available
                distance = -np.corrcoef(energy1_segment, segment)[0, 1]
    ...
    # Find best alignment (minimum distance for DTW, maximum correlation for fallback)
    if dtw is not None:
        best_idx = np.argmin(dtw_distances)  # Lower DTW distance is better
    else:
        best_idx = np.argmax(dtw_distances)  # Higher correlation is better (negative values)
    best_offset = offset_range[best_idx]
...
def main():
    ...
    if len(sys.argv) < 4 or len(sys.argv) > 5:
        ...
        print("  - reference_video: The reference video filename (choose the one with earliest matching pattern/most concise content)")
