...
CREATE TABLE IF NOT EXISTS musician_frame_analysis (
    ...
    -- Model information (replaces model_version)
    hand_model VARCHAR(50),      -- e.g., 'mediapipe_hands_v1.0', 'yolo_hand_v8'
    pose_model VARCHAR(50),      -- e.g., 'mediapipe_pose_v1.0', 'yolo11n-pose'
    facemesh_model VARCHAR(50),  -- e.g., 'mediapipe_facemesh_v1.0', 'yolo_face'
    emotion_model VARCHAR(50),   -- e.g., 'mediapipe_facemesh_v1.0', 'fer_v2.0'
    
    -- Analysis metadata  
    processing_time_ms INTEGER,                    -- Total processing time (kept for backward compatibility)
    hand_processing_time_ms INTEGER DEFAULT 0,     -- Time spent on hand detection
    pose_processing_time_ms INTEGER DEFAULT 0,     -- Time spent on pose detection
    facemesh_processing_time_ms INTEGER DEFAULT 0, -- Time spent on face mesh detection
    emotion_processing_time_ms INTEGER DEFAULT 0,  -- Time spent on emotion detection
